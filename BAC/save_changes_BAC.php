...
function updateStudentData_BAC($connection, $student_id, $column, $lastName, $firstName, $middleName, $phoneNumber, $description, $foreign_language, $rezults)
{
    ...
    // Проверка передаваемой колонки и выполнение соответствующего запроса
    if ($column === 'name') {
        // После успешного выполнения запроса на обновление данных студента
        $query = "UPDATE Students_BAC SET last_name = ?, first_name = ?, middle_name = ? WHERE student_id = ?";
        ...
        $query = "UPDATE Students_BAC SET phone_number = ? WHERE student_id = ?";
        $statement = mysqli_prepare($connection, $query);
        if ($statement) {
        ...
        $query = "UPDATE Students_BAC SET description = ? WHERE student_id = ?";
        $statement = mysqli_prepare($connection, $query);
        if ($statement) {
        ...
        $query = "UPDATE Students_BAC SET foreign_language = ? WHERE student_id = ?";
        $statement = mysqli_prepare($connection, $query);
        if ($statement) {
    ...
    elseif ($column === 'rezults') {
        $query = "UPDATE Students_BAC SET rezults = ? WHERE student_id = ?";
        $statement = mysqli_prepare($connection, $query);
        if ($statement) {
...
if (isset($_POST['column'])) {
    ...
    updateStudentData_BAC($connection, $student_id, $column, $lastName, $firstName, $middleName, $phoneNumber, $description, $foreign_language, $rezults);
}
?>
