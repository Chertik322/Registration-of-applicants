...
<body>
  ...
  <div class="collapse navbar-collapse" id="navbarNav">
    ...
    <form method="post" action="export_MAG.php"> <!-- Указываем путь к файлу обработчику -->
    ...
    <div class="table-responsive">
        <table class="table">
            ...
            <tbody id="students-table">
                <?php 
                 echo generate_students_table_MAG($students, $connection); ?>
            </tbody>
        </table>
    </div>
<?php else: ?>
...
function sortTable(column, link) {
  ...
  rows.sort(function(rowA, rowB) {
    ...
    var valueA = getStudentNumber_MAG(cellA.textContent.trim());
    var valueB = getStudentNumber_MAG(cellB.textContent.trim());
...
function getStudentNumber_MAG(cellValue) {
...
$(document).ready(function() {
    ...
    if (keycode == '13') {
      ...
      if (column === 'rezults') {
        if (value >= 0 && value <= 100) {
          updateStudentData_MAG(student_id, column, value);
        } else {
  ...
  function updateStudentData_MAG(student_id, column, value) {
    var lastName = '';
    var firstName = '';
    var middleName = '';
    var phoneNumber = '';
...
$(document).ready(function() {
  ...
  $('.btn-link').click(function() {
    ...
    $.ajax({
      url: 'details_MAG.php', // Замените на вашу обработчик запросов для получения данных о студенте
      method: 'POST',
...
<script>
function insertSpecializationToStudent_MAG(student_id, specialization_id) {
  $.ajax({
  url: 'add_specialization_MAG.php',
  type: 'POST',
  data: {
    student_id: student_id,
  ...
  function deleteSpecializationFromStudent_MAG(student_id, specialization_id) {
    $.ajax({
      url: 'delete_specialization_MAG.php',
      type: 'POST',
      data: {
        student_id: student_id,
        specialization_id: specialization_id
...
location.reload(true);
  ...
  $(document).ready(function() {
    ...
    $('.specialization-option').click(function(e) {
      ...
      insertSpecializationToStudent_MAG(student_id, specialization_id);
    ...
    $('.delete-specialization-option').click(function(e) {
      ...
      deleteSpecializationFromStudent_MAG(student_id, specialization_id);
      $(this).closest('.dropdown-menu').hide();
    });
  });
</script>
<script>
  function insertActionToLog_MAG(studentId, iconType) {
    var data = {
  ...
  for (var i = 0; i < actionLogIcons.length; i++) {
    actionLogIcons[i].addEventListener('click', function(event) {
      ...
      insertActionToLog_MAG(studentId, iconType);
    });
  }
</script>
<script>
